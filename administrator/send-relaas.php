...
$pesan = 'Ini Pesan RELAAS'; // Ambil pesan dari inputan form
$file_relaas = $_POST['file_relaas']; // Ambil nama file yang di upload
...
if(empty($file_relaas)){ // Jika tanpa file_relaas
    $send = $mail->send();
...
}else{ // Jika dengan file_relaas
    $tmp = $_POST['file_relaas'];
    $size = $_POST['file_relaas'];

    if($size <= 25000000){ // Jika ukuran file <= 25 MB (25.000.000 bytes)
        $mail->addAttachment($tmp, $file_relaas); // Add file yang akan di kirim
