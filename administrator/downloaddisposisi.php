...
// Mengambil data dari tabel
                            ...
                            $tgl_surat = $data['tgl_lporan'];
                            $tgl_surat = date('d/m/Y', strtotime($tgl_surat));
                            $tgl_masuk = $data['tgl_lporan'];
                            $tgl_masuk = date('d/m/Y', strtotime($tgl_masuk));
                            
                            $tahun = $data['tgl_lporan'];
                            $tahun =  date('Y', strtotime($tahun));
                            $nama_file = $tahun.'-'.$data['no_surat'].'-nama_pengadilan';
                            $bulan = $data['tgl_lporan'];
...
$SI = $excelku->setActiveSheetIndex(0);
 $SI->setCellValue('N1',$data['no_urut']);
 $SI->setCellValue('U1',$data['no_terima']);
 $SI->setCellValue('S1',$bulan);
 $SI->setCellValue('D3',$data['nama_pengadilan']);
 $SI->setCellValue('B8',$data['nama_js']);
 $SI->setCellValue('D11',$tgl_surat);
 $SI->setCellValue('B13',$data['wilayah_js']);
 $SI->setCellValue('K11',$data['keterangan']);
