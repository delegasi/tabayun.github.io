<?php
	...
	$nama_file_lengkap 		= $_FILES['file_relaas']['name'];
	...
	$tipe_file 		= $_FILES['file_relaas']['type'];
	$ukuran_file 	= $_FILES['file_relaas']['size'];
	$tmp_file 		= $_FILES['file_relaas']['tmp_name'];

    if (($tipe_file == "application/pdf") and ($ukuran_file <= 10340000)){	
        
		$nama_baru = $judul.'-'.$jam.$menit.$detik.'_'.$tglNow.$blnNow.$thnNow. $ext_file;
		$path = "../dokumen-relaas/file-relaas/".$nama_baru;
		move_uploaded_file($tmp_file, $path);
	
    //jika gambar tidak ada
	$sql = "UPDATE tb_delegasimasuk set file_relaas = '$nama_baru' where id='".$id."'";
