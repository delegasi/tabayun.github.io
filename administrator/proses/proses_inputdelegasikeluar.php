<?php
    ...
    if (!($tgl_keluar=='') and !($kode_suratkeluar =='') and !($nomor_suratkeluar =='') and !($nama_bagian =='') and !($tgl_surat =='') and !($kepada_suratkeluar =='') and !($perihal_suratkeluar =='') and !($operator =='') and !($tanggal_entry =='') and   
		...
		$execute = mysqli_query($db, $sql);
        		...
        		echo "<script>alert('Data Delegasi Keluar Berhasil Diinput)</script>";
        		echo "<meta http-equiv='refresh' content='0; url=../datasuratkeluar.php'>";
	}
	else{
        		echo "<script>alert('Maaf Data Yang Anda Masukan Kurang Lengkap')</script>";
        		echo "<meta http-equiv='refresh' content='0; url=../inputsuratkeluar.php'>";
	}
