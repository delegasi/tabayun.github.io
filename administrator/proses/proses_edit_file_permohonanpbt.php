...
$nama_file_lengkap = $_FILES['file_permo_pbt']['name'];
...
$tipe_file = $_FILES['file_permo_pbt']['type'];
$ukuran_file = $_FILES['file_permo_pbt']['size'];
$tmp_file = $_FILES['file_permo_pbt']['tmp_name'];

if (($tipe_file == "application/pdf") and ($ukuran_file <= 10340000)) {

    $nama_baru = $judul.'-'.$jam.$menit.$detik.'_'.$tglNow.$blnNow.$thnNow. $ext_file;
    $path = "../dokumen-permohonan/file-pbt/" . $nama_baru;
    move_uploaded_file($tmp_file, $path);

    //jika gambar tidak ada
    $sql = "UPDATE tb_delegasimasuk set file_permo_pbt = '$nama_baru' where id='" . $id . "'";
